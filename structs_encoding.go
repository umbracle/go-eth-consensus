...
// Hash: 87a0b50ee74483304b87bf036007496df43f8ac85d25588b6495a3a6b9f3c7dd
package consensus
...
func (b *BeaconStatePhase0) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(b.GenesisValidatorsRoot); size != 32 {
		err = ssz.ErrBytesLengthFn("BeaconStatePhase0.GenesisValidatorsRoot", size, 32)
		return
	}
	...
	if size := len(b.BlockRoots); size != 8192 {
		err = ssz.ErrVectorLengthFn("BeaconStatePhase0.BlockRoots", size, 8192)
		return
	}
	for ii := 0; ii < 8192; ii++ {
		if size := len(b.BlockRoots[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("BeaconStatePhase0.BlockRoots[ii]", size, 32)
			return
		}
	...
	if size := len(b.StateRoots); size != 8192 {
		err = ssz.ErrVectorLengthFn("BeaconStatePhase0.StateRoots", size, 8192)
		return
	}
	for ii := 0; ii < 8192; ii++ {
		if size := len(b.StateRoots[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("BeaconStatePhase0.StateRoots[ii]", size, 32)
			return
		}
	...
	if size := len(b.RandaoMixes); size != 65536 {
		err = ssz.ErrVectorLengthFn("BeaconStatePhase0.RandaoMixes", size, 65536)
		return
	}
	for ii := 0; ii < 65536; ii++ {
		if size := len(b.RandaoMixes[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("BeaconStatePhase0.RandaoMixes[ii]", size, 32)
			return
		}
	...
	if size := len(b.JustificationBits); size != 1 {
		err = ssz.ErrBytesLengthFn("BeaconStatePhase0.JustificationBits", size, 1)
		return
	}
	...
	for ii := 0; ii < len(b.HistoricalRoots); ii++ {
		if size := len(b.HistoricalRoots[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("BeaconStatePhase0.HistoricalRoots[ii]", size, 32)
			return
		}
